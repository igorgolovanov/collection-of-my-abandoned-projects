...
/**
 * This filter reports an error if the applied action is receiving a non-PATCH request.
 * 
 ...
 * @category filters
 */
class RestfulPatchOnlyFilter extends CFilter
{
    ...
    protected function preFilter($filterChain)
    {
        ...
        if (method_exists($request, 'getIsPatchRequest')) {
            if ($request->getIsPatchRequest()) {
                return true;
            }
        } elseif ($request instanceof CComponent && $request->asa('restful') instanceof RestfulHttpRequestBehavior) {
            if ($request->asa('restful')->getIsPatchRequest()) {
                return true;
            }
        } elseif ('PATCH' === $request->getRequestType()) {
            return true;
        }
