...
/**
 * This filter reports an error if the applied action is receiving a non-OPTIONS request.
 * 
 ...
 * @category filters
 */
class RestfulOptionsOnlyFilter extends CFilter
{
    ...
    protected function preFilter($filterChain)
    {
        ...
        if (method_exists($request, 'getIsOptionsRequest')) {
            if ($request->getIsOptionsRequest()) {
                return true;
            }
        } elseif ($request instanceof CComponent && $request->asa('restful') instanceof RestfulHttpRequestBehavior) {
            if ($request->asa('restful')->getIsOptionsRequest()) {
                return true;
            }
        } elseif ('OPTIONS' === $request->getRequestType()) {
            return true;
        }
