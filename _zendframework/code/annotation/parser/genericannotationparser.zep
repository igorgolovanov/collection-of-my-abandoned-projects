...
class GenericAnnotationParser implements ParserInterface
{
    ...
    {
        var className, content, index, annotation, annotationNew;

        let className = e->getParam("class", false);
        if !className || !this->hasAnnotation(className) {
            return false;
        }
        ...
        let annotationNew = clone annotation; // todo: ?? <AnnotationInterface>
    ...
    public function registerAnnotation(var annotation) -> <GenericAnnotationParser>
    {
        ...
        if typeof annotation == "string" && class_exists(annotation) {
            ...
            let annotation = new {annotation}(); // todo: <AnnotationInterface>
        }
