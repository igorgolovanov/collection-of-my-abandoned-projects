...
ZEPHIR_INIT_CLASS(ZendFramework_Stdlib_SplStack) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Zend\\Stdlib, SplStack, zendframework, stdlib_splstack, spl_ce_SplStack, zendframework_stdlib_splstack_method_entry, 0);

	zend_class_implements(zendframework_stdlib_splstack_ce TSRMLS_CC, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(ZendFramework_Stdlib_SplStack, toArray) {
	...
	for (;_0->funcs->valid(_0 TSRMLS_CC) == SUCCESS && !EG(exception); _0->funcs->move_forward(_0 TSRMLS_CC)) {
		...
		zephir_array_append(&data, item, PH_SEPARATE, "zendframework/stdlib/splstack.zep", 27);
	}
...
PHP_METHOD(ZendFramework_Stdlib_SplStack, unserialize) {
	...
	zephir_check_call_status();
	zephir_is_iterable(unserialized, &_2, &_1, 0, 0, "zendframework/stdlib/splstack.zep", 61);
	for (
