...
class EventManager implements EventManagerInterface
{
        ...
        if event instanceof EventInterface {
            ...
            if target instanceof EventInterface {
                ...
                if argv instanceof EventInterface {
                    ...
                    let e = new {eventClass}(); // todo: <EventInterface>
                    e->setName(event);
                    e->setTarget(target);
                    e->setParams(argv);
                }
    ...
    public function attach(var event, var callback = null, int priority = 1)
    {
        ...
        if unlikely callback === null {
            let exceptionMsg = __METHOD__ . ": expects a callback; none provided";
    ...
    {
        aggregate->attach(this, priority); // todo: check return.
    }
    ...
    {
        string exceptionMsg;
        var event, events, returns, expected;

        if listener instanceof ListenerAggregateInterface {
            return this->detachAggregate(listener);
        }

        if unlikely !(listener instanceof CallbackHandler) {
    ...
    {
        aggregate->detach(this); // todo: check return
    }
    ...
    protected function triggerListeners(string event, <EventInterface> e, callback = null) -> <ResponseCollection>
    {
        var responses, listeners, wildcardListeners, listener, last,
            ...
            sharedListeners, sharedWildcardListeners;
