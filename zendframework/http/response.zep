/*

This file is part of the php-ext-zendframework package.

For the full copyright and license information, please view the LICENSE
file that was distributed with this source code.

*/

namespace Zend\Http;


use Zend\Stdlib\ErrorHandler;
use Zend\Stdlib\ResponseInterface;

/**
 * HTTP Response
 *
 * @link      http://www.w3.org/Protocols/rfc2616/rfc2616-sec6.html#sec6
 */
class Response extends AbstractMessage implements ResponseInterface
{
    /**#@+
     * @const int Status codes
     */
    const STATUS_CODE_CUSTOM = 0;
    const STATUS_CODE_100 = 100;
    const STATUS_CODE_101 = 101;
    const STATUS_CODE_102 = 102;
    const STATUS_CODE_200 = 200;
    const STATUS_CODE_201 = 201;
    const STATUS_CODE_202 = 202;
    const STATUS_CODE_203 = 203;
    const STATUS_CODE_204 = 204;
    const STATUS_CODE_205 = 205;
    const STATUS_CODE_206 = 206;
    const STATUS_CODE_207 = 207;
    const STATUS_CODE_208 = 208;
    const STATUS_CODE_300 = 300;
    const STATUS_CODE_301 = 301;
    const STATUS_CODE_302 = 302;
    const STATUS_CODE_303 = 303;
    const STATUS_CODE_304 = 304;
    const STATUS_CODE_305 = 305;
    const STATUS_CODE_306 = 306;
    const STATUS_CODE_307 = 307;
    const STATUS_CODE_400 = 400;
    const STATUS_CODE_401 = 401;
    const STATUS_CODE_402 = 402;
    const STATUS_CODE_403 = 403;
    const STATUS_CODE_404 = 404;
    const STATUS_CODE_405 = 405;
    const STATUS_CODE_406 = 406;
    const STATUS_CODE_407 = 407;
    const STATUS_CODE_408 = 408;
    const STATUS_CODE_409 = 409;
    const STATUS_CODE_410 = 410;
    const STATUS_CODE_411 = 411;
    const STATUS_CODE_412 = 412;
    const STATUS_CODE_413 = 413;
    const STATUS_CODE_414 = 414;
    const STATUS_CODE_415 = 415;
    const STATUS_CODE_416 = 416;
    const STATUS_CODE_417 = 417;
    const STATUS_CODE_418 = 418;
    const STATUS_CODE_422 = 422;
    const STATUS_CODE_423 = 423;
    const STATUS_CODE_424 = 424;
    const STATUS_CODE_425 = 425;
    const STATUS_CODE_426 = 426;
    const STATUS_CODE_428 = 428;
    const STATUS_CODE_429 = 429;
    const STATUS_CODE_431 = 431;
    const STATUS_CODE_500 = 500;
    const STATUS_CODE_501 = 501;
    const STATUS_CODE_502 = 502;
    const STATUS_CODE_503 = 503;
    const STATUS_CODE_504 = 504;
    const STATUS_CODE_505 = 505;
    const STATUS_CODE_506 = 506;
    const STATUS_CODE_507 = 507;
    const STATUS_CODE_508 = 508;
    const STATUS_CODE_511 = 511;
    //**#@-*/


    /**
     * @var array Recommended Reason Phrases
     */
    protected recommendedReasonPhrases = [
		// INFORMATIONAL CODES
        100: "Continue",
        101: "Switching Protocols",
        102: "Processing",
        // SUCCESS CODES
        200: "OK",
        201: "Created",
        202: "Accepted",
        203: "Non-Authoritative Information",
        204: "No Content",
        205: "Reset Content",
        206: "Partial Content",
        207: "Multi-status",
        208: "Already Reported",
        // REDIRECTION CODES
        300: "Multiple Choices",
        301: "Moved Permanently",
        302: "Found",
        303: "See Other",
        304: "Not Modified",
        305: "Use Proxy",
        306: "Switch Proxy", // Deprecated
        307: "Temporary Redirect",
        // CLIENT ERROR
        400: "Bad Request",
        401: "Unauthorized",
        402: "Payment Required",
        403: "Forbidden",
        404: "Not Found",
        405: "Method Not Allowed",
        406: "Not Acceptable",
        407: "Proxy Authentication Required",
        408: "Request Time-out",
        409: "Conflict",
        410: "Gone",
        411: "Length Required",
        412: "Precondition Failed",
        413: "Request Entity Too Large",
        414: "Request-URI Too Large",
        415: "Unsupported Media Type",
        416: "Requested range not satisfiable",
        417: "Expectation Failed",
        418: "I\"m a teapot",
        422: "Unprocessable Entity",
        423: "Locked",
        424: "Failed Dependency",
        425: "Unordered Collection",
        426: "Upgrade Required",
        428: "Precondition Required",
        429: "Too Many Requests",
        431: "Request Header Fields Too Large",
        // SERVER ERROR
        500: "Internal Server Error",
        501: "Not Implemented",
        502: "Bad Gateway",
        503: "Service Unavailable",
        504: "Gateway Time-out",
        505: "HTTP Version not supported",
        506: "Variant Also Negotiates",
        507: "Insufficient Storage",
        508: "Loop Detected",
        511: "Network Authentication Required"
	];

    /**
     * @var int Status code
     */
    protected statusCode = 200;

    /**
     * @var string|null Null means it will be looked up from the $reasonPhrase list above
     */
    protected reasonPhrase;

    /**
     * Populate object from string
     *
     * @param  string $string
     * @return self
     * @throws Exception\InvalidArgumentException
     */
    public static function fromString(string $string) -> <Response>
    {
        var lines, firstLine, response, className, matches = [],
            reason, content, headers, line;
        string regex;
        boolean isHeader = true;

        let lines = explode("\r\n", $string);
        if typeof lines != "array" || count(lines) == 1 {
            let lines = explode("n", $string);
        }

        let firstLine = array_shift(lines);
        let className = get_called_class();
        let response = new {className}();

        let regex = "/^HTTP\/(?P<version>1\.[01]) (?P<status>\d{3})(?:[ ]+(?P<reason>.*))?$/";

        if unlikely !preg_match(regex, firstLine, matches) {
            throw new Exception\InvalidArgumentException("A valid response status line was not found in the provided string");
        }

        let response->version = matches["version"];

        response->setStatusCode(matches["status"]);

        if fetch reason, matches["reason"] {
            response->setStatusCode(reason);
        } else {
            response->setStatusCode("");
        }

        if count(lines) == 0 {
            return response;
        }

        let content = [];
        let headers = [];

        for line in lines {
            if isHeader && line == "" {
                let isHeader = false;
                continue;
            }
            if isHeader {
                let headers[] = line;
            } else {
                let content[] = line;
            }
        }

        if headers {
            let response->headers = implode("\r\n", headers);
        }
        if content {
            let content = implode("\r\n", content);
            response->setContent(content);
        }
        return response;
    }

    /**
     * @return Header\SetCookie[]
     */
    public function getCookie() -> array
    {
        var headers, cookie;

        let headers = <Headers> this->getHeaders();
        let cookie = headers->get("Set-Cookie");

        return cookie;
    }

    /**
     * Set HTTP status code and (optionally) message
     *
     * @param  int $code
     * @throws Exception\InvalidArgumentException
     * @return self
     */
    public function setStatusCode(var code) -> <Response>
    {
        var className, constName;
        string exceptionMsg;

        let className = get_class(this);
        let constName = className . "::STATUS_CODE_" . code;

        if unlikely !is_numeric(code) || !defined(constName) {
            let exceptionMsg = "Invalid status code provided: \"";
            if !is_scalar(code) {
                let exceptionMsg .= gettype(code);
            } else {
                let exceptionMsg .= code;
            }
            let exceptionMsg .= "\"";
            throw new Exception\InvalidArgumentException(exceptionMsg);
        }

        let this->statusCode = (int) code;

        return this;
    }

    /**
     * Retrieve HTTP status code
     *
     * @return int
     */
    public function getStatusCode() -> int
    {
        return this->statusCode;
    }

    /**
     * Set custom HTTP status code
     *
     * @param  int $code
     * @throws Exception\InvalidArgumentException
     * @return self
     */
    public function setCustomStatusCode(var code) -> <Response>
    {
        string exceptionMsg;

        if unlikely !is_numeric(code) {
            let exceptionMsg = "Invalid status code provided: \"";
            if !is_scalar(code) {
                let exceptionMsg .= gettype(code);
            } else {
                let exceptionMsg .= code;
            }
            let exceptionMsg .= "\"";
            throw new Exception\InvalidArgumentException(exceptionMsg);
        }

        let this->statusCode = (int) code;

        return this;
    }

    /**
     * @param string $reasonPhrase
     * @return self
     */
    public function setReasonPhrase(string reasonPhrase) -> <Response>
    {
        let this->reasonPhrase = reasonPhrase->trim();
        return this;
    }

    /**
     * Get HTTP status message
     *
     * @return string
     */
    public function getReasonPhrase() -> string
    {
        var reasonPhrase, statusCode;

        if this->reasonPhrase === null {
            let statusCode = this->statusCode;
            if fetch reasonPhrase, this->recommendedReasonPhrases[statusCode] {
                return reasonPhrase;
            }
        }
        return this->reasonPhrase;
    }

    /**
     * Get the body of the response
     *
     * @return string
     */
    public function getBody() -> string
    {
        var body, headers, transferEncoding, transferEncodingValue, contentEncoding;

        let body = (string) this->getContent();
        let headers = <Headers> this->getHeaders();
        let transferEncoding = headers->get("Transfer-Encoding");

        if !empty transferEncoding {
            let transferEncodingValue = transferEncoding->getFieldValue();
            if strtolower(transferEncodingValue) == "chunked" {
                let body = this->decodeChunkedBody(body);
            }
        }

        let contentEncoding = headers->get("Content-Encoding");

        if !empty contentEncoding {
            let contentEncoding = contentEncoding->getFieldValue();

            if contentEncoding == "gzip" {
                let body = this->decodeGzip(body);
            } elseif contentEncoding == "deflate" {
                let body = this->decodeDeflate(body);
            }
        }

        return body;
    }

    /**
     * Does the status code indicate a client error?
     *
     * @return bool
     */
    public function isClientError() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code < 500 && code >= 400;
    }

    /**
     * Is the request forbidden due to ACLs?
     *
     * @return bool
     */
    public function isForbidden() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code == 403;
    }

    /**
     * Is the current status "informational"?
     *
     * @return bool
     */
    public function isInformational() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code >= 100 && code < 200;
    }

    /**
     * Does the status code indicate the resource is not found?
     *
     * @return bool
     */
    public function isNotFound() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code == 404;
    }

    /**
     * Do we have a normal, OK response?
     *
     * @return bool
     */
    public function isOk() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code == 200;
    }

    /**
     * Does the status code reflect a server error?
     *
     * @return bool
     */
    public function isServerError() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code >= 500 && code < 600;
    }

    /**
     * Do we have a redirect?
     *
     * @return bool
     */
    public function isRedirect() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code >= 300 && code < 400;
    }

    /**
     * Was the response successful?
     *
     * @return bool
     */
    public function isSuccess() -> boolean
    {
        var code;

        let code = this->getStatusCode();

        return code >= 200 && code < 300;
    }

    /**
     * Render the status line header
     *
     * @return string
     */
    public function renderStatusLine() -> string
    {
        var version, code, phrase;
        string status;

        let version = this->getVersion();
        let code = this->getStatusCode();
        let phrase = this->getReasonPhrase();
        let status = (string) sprintf("HTTP/%s %d %s", version, code, phrase);

        return status->trim();
    }

    /**
     * Render entire response as HTTP response string
     *
     * @return string
     */
    public function toString() -> string
    {
        var statusLine, headers, headersStr, content;
        string str;

        let statusLine = this->renderStatusLine();
        let content = this->getContent();
        let headers = <Headers> this->getHeaders();
        let headersStr = headers->toString();

        let str = statusLine . "\r\n" . headersStr . "\r\n" . content;

        return str;
    }

    /**
     * Decode a "chunked" transfer-encoded body and return the decoded text
     *
     * @param  string $body
     * @return string
     * @throws Exception\RuntimeException
     */
    protected function decodeChunkedBody(string body) -> string
    {
        var m = [], lengthHex, length, cut;
        string decBody = "";

        while body->trim() {
            if unlikely !preg_match("/^([\da-fA-F]+)[^\r\n]*\r\n/sm", body, m) {
                throw new Exception\RuntimeException("Error parsing body - doesn't seem to be a chunked message");
            }
            let lengthHex = trim(m[1]);
            let length = hexdec(lengthHex);
            let cut = strlen(m[0]);
            let decBody .= substr(body, cut, length);
            let body = substr(body, cut + length + 2);
        }

        return decBody;
    }

    /**
     * Decode a gzip encoded message (when Content-encoding = gzip)
     *
     * Currently requires PHP with zlib support
     *
     * @param  string $body
     * @return string
     * @throws Exception\RuntimeException
     */
    protected function decodeGzip(string body) -> string
    {
        var test, bodyCut, result;

        if unlikely !function_exists("gzinflate") {
            throw new Exception\RuntimeException("zlib extension is required in order to decode \"gzip\" encoding");
        }

        ErrorHandler::start();
        let bodyCut = substr(body, 10);
        let result = gzinflate(bodyCut);
        let test = ErrorHandler::stop();

        if unlikely !empty test {
            throw new Exception\RuntimeException("Error occurred during gzip inflation", 0, test);
        }
        return result;
    }

    /**
     * Decode a zlib deflated message (when Content-encoding = deflate)
     *
     * Currently requires PHP with zlib support
     *
     * @param  string $body
     * @return string
     * @throws Exception\RuntimeException
     */
    protected function decodeDeflate(string body) -> string
    {
        var result, zlibHeader, zlibHeaderFirst, bodyCut;

        if unlikely !function_exists("gzuncompress") {
            throw new Exception\RuntimeException("zlib extension is required in order to decode \"deflate\" encoding");
        }
        /**
         * Some servers (IIS ?) send a broken deflate response, without the
         * RFC-required zlib header.
         *
         * We try to detect the zlib header, and if it does not exist we
         * teat the body is plain DEFLATE content.
         *
         * This method was adapted from PEAR HTTP_Request2 by (c) Alexey Borzov
         *
         * @link http://framework.zend.com/issues/browse/ZF-6040
         */
         let bodyCut = substr(body, 0, 2);
         let zlibHeader = unpack("n", bodyCut);
         let zlibHeaderFirst = zlibHeader[1];

         if zlibHeaderFirst % 31 == 0 {
            let result = gzuncompress(body);
         } else {
            let result = gzinflate(body);
         }
        return result;
    }

}
