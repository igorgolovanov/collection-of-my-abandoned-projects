...
class ArrayObject implements IteratorAggregate, ArrayAccess, Serializable, Countable
{
     ...
     */
    public function __construct(array input = [], int flags = 1, string iteratorClass = "ArrayIterator")
    {
        this->setFlags(flags);
    ...
    public function __unset(var key) -> void
    {
       ...
       // unset this->{key}; // todo: property-dynamic-access in "unset"
    }
     ...
     * @return mixed
     */
    public function __get(var key) // todo: add &
    {
        string exceptionMsg;
        var ret;

        if this->flag == self::ARRAY_AS_PROPS {
            let ret = this->offsetGet(key); // todo:  =&
            return ret;
        }
        if unlikely in_array(key, this->protectedProperties) {
            let exceptionMsg = "\"" . key . "\" is a protected property, use a different key";
    ...
    public function exchangeArray(var data) -> array
    {
        ...
        if type == "object" && (data instanceof ArrayObject || data instanceof \ArrayObject) {
    ...
    {
        var className, iterator, storage;

        let storage = this->storage;
        let className = this->iteratorClass;
        let iterator = new {className}(storage);
     ...
     * @return mixed
     */
    public function offsetGet(key) // todo: add &
    {
        var ret = null;
    ...
    public function unserialize(string data) -> void
    {
        ...
        if typeof unserialized == "object" {
            let unserialized = iterator_to_array(unserialized);
        }
