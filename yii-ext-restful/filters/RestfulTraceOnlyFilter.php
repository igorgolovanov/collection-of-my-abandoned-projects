...
/**
 * This filter reports an error if the applied action is receiving a non-TRACE request.
 * 
 ...
 * @category filters
 */
class RestfulTraceOnlyFilter extends CFilter
{
    ...
    protected function preFilter($filterChain)
    {
        ...
        if (method_exists($request, 'getIsTraceRequest')) {
            if ($request->getIsTraceRequest()) {
                return true;
            }
        } elseif ($request instanceof CComponent && $request->asa('restful') instanceof RestfulHttpRequestBehavior) {
            if ($request->asa('restful')->getIsTraceRequest()) {
                return true;
            }
        } elseif ('TRACE' === $request->getRequestType()) {
            return true;
        }
