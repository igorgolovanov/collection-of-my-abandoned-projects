...
class LdapException extends \Exception implements ExceptionInterface
{
     ...
     */
    public function __construct(var ldap = null, string str = null, int code = 0)
    {
        // todo: ldap type Ldap
        var errors = [], message = "", oldCode;

        if ldap !== null {
            let oldCode = code;
            let message = ldap->getLastError(code, errors) . ": ";
            if code === 0 {
                let message = "";
                let code = oldCode;
            }
        ...
        if !empty str {
            ...
            let message .= "no exception message";
        }
