<?php
/*
Plugin Name: WP Transliterations
Plugin URI: http://github.com/golovanov/wp-transliterations/
Description: Transliterates UTF-8 encoded text/slug/name to US-ASCII.
Version: 0.1
Author: Igor Golovanov
Author URI: http://github.com/golovanov/
*/

defined( 'WP_TRANSLITERATIONS_DIRECTORY' ) or define( 'WP_TRANSLITERATIONS_DIRECTORY', __DIR__ . '/transliterations' );
 ...
 * Transliterated text.
 */
function wp_transliterate(
	$string, $unknown = '-',
	$source_langcode = 'UTF-8'
) {
	...
	preg_match_all( '/[\x00-\x7f]+|[\x80-\xff][\x00-\x40\x5b-\x5f\x7b-\xff]*/',
		$string, $matches );
	...
	foreach ( $matches[0] as $str ) {
		...
		for ( $i = - 1; --$len; ) {
			...
			if ( $remaining = $tail_bytes[ $c ] ) {
				...
				$result .= _wp_transliterate_replace( $ord, $unknown,
					$source_langcode );
				$head = '';
			} elseif ( $c < "\x80" ) {
				// ASCII byte.
...
function _wp_transliterate_replace(
	$ord, $unknown = '?',
	$langcode = 'UTF-8'
) {
	static $map = array();
	...
	if ( ! isset( $map[ $bank ][ $langcode ] ) ) {
		$file = WP_TRANSLITERATIONS_DIRECTORY . DIRECTORY_SEPARATOR . sprintf( 'x%02x',
				$bank ) . '.php';
		if ( file_exists( $file ) ) {
			include $file;
...
function wp_transliterate_sanitize_title( $title ) {
	return wp_transliterate( seems_utf8( $title ) ? urldecode( $title ) : $title );
}

add_filter( 'sanitize_file_name', 'wp_transliterate_sanitize_title' );
add_filter( 'sanitize_title', 'wp_transliterate_sanitize_title' );
