...
ZEPHIR_INIT_CLASS(OAuth_TokenNotFoundException) {
	...
	ZEPHIR_REGISTER_CLASS_EX(OAuth, TokenNotFoundException, oauth, tokennotfoundexception, zend_exception_get_default(TSRMLS_C), oauth_tokennotfoundexception_method_entry, 0);
...
/**
	 ...
	 * @var string
	 */
	zend_declare_property_null(oauth_tokennotfoundexception_ce, SL("token"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_class_implements(oauth_tokennotfoundexception_ce TSRMLS_CC, 1, oauth_exceptioninterface_ce);
...
PHP_METHOD(OAuth_TokenNotFoundException, __construct) {
	...
	ZEPHIR_INIT_VAR(_0);
	ZVAL_LONG(_0, code);
	zephir_call_parent_p3_noret(this_ptr, oauth_tokennotfoundexception_ce, "__construct", message, _0, previous);
